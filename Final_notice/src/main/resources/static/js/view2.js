	...
	$('ul+.commnet-write .btn-register').click(function(){
		...
		$.ajax({
			url: '../inquirycomment/add',
			data: {
				id: $("#loginid").text(),
				content : content,
				comment_inquery_num : $("#comment_inquery_num").val(),
				comment_re_lev: 0,
				comment_re_seq:0
			},
			type: 'post',
	...
	$('.comment-area').on('click','.reply',function(){
		...
		console.log("lev는 "+lev);
		console.log("seq는 "+seq);
		
		$.ajax({
			url : '../inquirycomment/reply',  
			data : {
				id : $("#loginid").text(),
				content : content,
				comment_inquery_num : $("#comment_inquery_num").val(),
...
function getList(state,page){
	...
	$.ajax({
		type:"post",
		url:"../inquirycomment/list",
		data: {"comment_inquery_num" : $("#comment_inquery_num").val(),
			   state: state,
			   page: page},
		dataType:"json",
		beforeSend : function(xhr){
		...
		success:function(rdata){
			...
			if(rdata.list.length>0){
				...
				+ '			 <a href="javascript:getList(1,1)" class="comment-order-button">등록순</a>'
				+		  '</li>'
				+		   '<li class="comment-order-item '+red2+'">'
				+ '			 <a href="javascript:getList(2,1)" class="comment-order-button">최신순</a>'
				+		  '</li>';
				...
				if(rdata.listcount>rdata.list.length) {
					output += '<li id="8" class="comment-list-item"><a href="javascript:getList('+option+','+ (page+1) +')">더보기</a></li>'
				}
